...
import config
import bot
register_user_if_not_exists = bot.register_user_if_not_exists
db = bot.db
logger = bot.logger
user_semaphores = bot.user_semaphores
...
async def message_handle(update: Update, context: CallbackContext, message=None, use_new_dialog_timeout=True):
    # check if bot was mentioned (for group chats) 在群组里，用户是否被提及，没有则退出。
    if not await bot.is_bot_mentioned(update, context):
        return
