...
class ChatGPT:
    ...
    # 向 gpt 发送问题，并返回回答。需要参数：问题的内容、聊天记录，对gpt角色的预设
    async def send_message(self, message, dialog_messages=[], chat_mode="assistant"):
        # 必须是chat_mode中的预设角色
        if chat_mode not in config.chat_modes.keys():
            raise ValueError(f"Chat mode {chat_mode} is not supported")

        # dialog_messages：对话的长度，即所有的聊天记录
        ...
        while answer is None:
            try:
                # 如果是 gpt-3.5-turbo 或 gpt-4，则使用 _generate_prompt_messages 函数生成消息列表，
                # 并使用 openai.ChatCompletion.acreate 方法进行聊天。
                ...
                # 如果是 text-davinci-003，则使用 _generate_prompt 函数生成提示信息，
                # 并使用 openai.Completion.acreate 方法进行聊天。
                ...
                # forget first message in dialog_messages. 它将从 dialog_messages 中删除第一个消息，并重试。
...
async def get_short_summary(text):
    prompt = config.chat_modes["short_summary"]["prompt_start"]
    # 加上语音转的文字，生成新的prompt
    prompt += "/n---"
    prompt += text
    prompt += "/n---"
